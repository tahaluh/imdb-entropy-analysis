...
INPUT_CSV = PROCESSED / "movies.csv"
...
def plot_rating_distribution(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    plt.title("Distribuicao de notas IMDb")
    plt.xlabel("Nota")
...
def plot_movies_per_year(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "movies_per_year.png", dpi=180)
    plt.close()
...
def plot_rating_by_year(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    # Keep years with at least 10 movies to reduce noise in the trend lines.
    ...
    plt.title("Nota por ano (media e mediana)")
    plt.xlabel("Ano")
    plt.ylabel("Nota")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_dir / "rating_by_year.png", dpi=180)
    plt.close()
...
def plot_votes_vs_rating(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    plt.xscale("log")
    plt.title("Relacao votos x nota (escala log em votos)")
    plt.xlabel("Votos (log)")
    plt.ylabel("Nota")
    plt.tight_layout()
    plt.savefig(out_dir / "votes_vs_rating.png", dpi=180)
    plt.close()
...
def main() -> None:
    ...
    if not INPUT_CSV.exists():
        raise FileNotFoundError(
            f"Arquivo nao encontrado: {INPUT_CSV}. Rode primeiro o 01_collect_imdb.py"
        )
